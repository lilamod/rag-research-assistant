...
# Simple response cache: cleared when documents change
...
def answer_question_stream(question: str, top_k: int = None):
    """Retrieve relevant chunks, then yield answer tokens via streaming.

    Yields (token, None) for each answer token,
    then (None, sources_list) when streaming is done.
    """
    top_k = top_k or settings.TOP_K
    query_vector = embed_query(question)
    results = store.search(query_vector, top_k=top_k)
